...
int ros_node_init(struct ros_node *ros_node)
{
  ...
  // create context
  rc = rcl_init(0, 0, &init_options, &ros_node->context);
  if (rc != RCL_RET_OK) {
  ...
  rc = rcl_node_init(&ros_node->node, "node_0", "executor_examples", &ros_node->context, &node_ops);
  if (rc != RCL_RET_OK) {
...
void ros_node_destroy(struct ros_node *ros_node)
{
    rcl_node_fini(&ros_node->node);
    rcl_init_options_fini(&ros_node->init_options);
}